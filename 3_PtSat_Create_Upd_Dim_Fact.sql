...
---===ADMNSRVY_D - UPDATE WITH DATA FROM DIM_ADMNSRVY_D 
...
---===VISITTYPE- UPDATE WITH DATA FROM XWALK FOR VISITTYPE =======

TRUNCATE TABLE SRVY_CODE_XWALK
/

INSERT INTO SRVY_CODE_XWALK
SELECT * FROM HDI.SRVY_CODE_XWALK
/
COMMIT
/
...
DECLARE
	...
	SELECT DISUNIT, VISITTYPE, ROWID
	FROM FACT_STG_SRVY_QSTN_RSPNS_F
	where length(disunit) = 5
	and visittype3 is null;
	V_DISUNIT VARCHAR2(200);
	V_VISITTYPE VARCHAR2(200);
	V_ROWID ROWID;

BEGIN
	OPEN C_COST_CTR;
	LOOP
	FETCH C_COST_CTR INTO V_DISUNIT, V_VISITTYPE, V_ROWID;
	EXIT WHEN C_COST_CTR%NOTFOUND;

	UPDATE FACT_STG_SRVY_QSTN_RSPNS_F
	SET (FACILITY, FINAL_UNIT_NAME, SURVEY_TYPE,
			sample_unit, survey_code, patient_type)  = 
	(SELECT  FACILITY, OLD_FINAL_UNIT_NAME, SURVEY_TYPE,
			sample_unit, survey_code, patient_type
		FROM HDI.SRVY_CODE_XWALK 
			WHERE COST_CENTER = V_DISUNIT
			AND VISIT_TYPE = V_VISITTYPE
			and rownum = 1)
  	where ROWID = V_ROWID;
	END LOOP;
	CLOSE C_COST_CTR;
...
---===VISITTYPE2 UPDATE WITH DATA FROM REV_CODE_REF FOR VISITTYPE2 =========

DECLARE
	CURSOR C_REV_CODE IS 
	SELECT DISUNIT, VISITTYPE_MODIFIED, ROWID
	FROM FACT_STG_SRVY_QSTN_RSPNS_F
	WHERE VISITTYPE3 IS NULL
	and length(disunit) = 4;
...
--===VISITTYPE3 - UPDATE WITH DATA FROM REV_CODE_REF FOR VISITYTYPE3 =====================
...
---==== Facility_D (UPDATE FACT STG WITH DATA FROM DIM_FACILITY_D)  ========================
...
---== Question Number - Add to Stg Fact FROM DIM_SRVY_QSTN_D   ============
...
---===RATINGS USING QUERY RATING SPREADSHEET FROM FENGWEI ======
